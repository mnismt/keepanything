/**
 * Mirror of `src/renderer/src/styles/themes.ts`. Kept as a real file (not a symlink) because the
 * desktop version imports `Theme` from `../../../shared/types`, which the web does not depend on.
 * The `Theme` type is inlined here; everything else stays byte-identical to the source of truth.
 * `tokens.stylex.ts` and `shared.ts` are symlinks to the same source files.
 */
import * as stylex from '@stylexjs/stylex'
import { colors, shadows } from './tokens.stylex'

/** User theme preference. Mirrors `Theme` in `src/shared/types.ts` of the desktop app. */
